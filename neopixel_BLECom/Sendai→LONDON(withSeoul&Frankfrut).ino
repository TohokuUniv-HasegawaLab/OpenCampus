...
void loop() {
  ...
  if (hasCmd) {
    ...
    if (cmd == 0x02) blinkAll  (r, g, b, dur, px);
    else if (cmd == 0x03) {
      if (RouteId == 10) {
          chaseSegment(0, 128, 255, dur, px, 0, SEOUL_LED); //SENDAI→SEOUL
        } 
        else if (RouteId == 11) {
          // 15..末尾 を緑
          chaseSegment(0, 255, 64, dur, px, FRANKFURT_LED, NUM_LEDS); //Frankfrut→LONDON
        } 
        else {
          // 従来の全範囲（色固定なら上書き済みの r,g,b を使用）
          chase(r, g, b, dur, px, RouteId);
        }
      }
