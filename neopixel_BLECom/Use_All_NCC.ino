...
// ==== ハード設定 ====
#define LED_PIN     5
#define NUM_LEDS    37 //LEDストリップの全長
#define RX_LED_PIN  2   // 受信インジケータ用。L基板に合わせて変更可（無ければ外す）

// ==== BLE設定 ====
static const char* DEVICE_NAME  = "MB→LO";
...
void chase(uint8_t r, uint8_t g, uint8_t b, uint16_t duration_ms, uint16_t step_delay_ms) {
  unsigned long t0 = millis();
  int pos = 0;
  while ((millis() - t0) < duration_ms) {
  ledsOff();
...
void chaseSegment(uint8_t r, uint8_t g, uint8_t b,
                  uint16_t duration_ms, uint16_t step_delay_ms,
  ...
  while ((millis() - t0) < duration_ms) {
    // 範囲内のみ点灯
    for (int i = 0; i < NUM_LEDS; i++) strip.setPixelColor(i, 0);
    strip.setPixelColor(startIdx + pos, strip.Color(r, g, b));
    strip.show();
    pos = (pos + 1) % len;
    delay(step_delay_ms);
  }
...
void loop() {
  if (hasCmd) {
    ...
    if (cmd == 0x03) {chase(r, g, b, dur, px);
    } else if (cmd == 0x01) {
      solidColor(r, g, b, dur);
    } else if (cmd == 0x02) {
      blinkAll(r, g, b, dur, px);
    }
  }
}
